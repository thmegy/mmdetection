...
class DeformableDETRHeadLPM(DeformableDETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 weight=1.,
                 **kwargs):
        super(DeformableDETRHeadLPM, self).__init__(*args, with_box_refine=with_box_refine, as_two_stage=as_two_stage, transformer=transformer, **kwargs)
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox, losses_iou, cls_avg_factor, num_total_pos = multi_apply(
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox, enc_losses_iou, enc_cls_avg_factor, enc_num_total_pos = \
                self.loss_single(enc_cls_scores, enc_bbox_preds,
                                 gt_bboxes_list, binary_labels_list,
                                 img_metas, gt_bboxes_ignore)
            # compute losses per event: mean over bboxes
            enc_loss_cls = enc_loss_cls.sum(dim=(1,2)) / enc_cls_avg_factor
            enc_losses_bbox = enc_losses_bbox.sum(dim=(1,2)) / enc_num_total_pos
            enc_losses_iou = enc_losses_iou.sum(dim=-1) / enc_num_total_pos
        
        # compute losses per event: mean over bboxes, then sum over decode layers
        losses_cls = sum([_loss.sum(dim=(1,2)) / avg_factor for _loss, avg_factor in zip(losses_cls, cls_avg_factor)])
        losses_bbox = sum([_loss.sum(dim=(1,2)) / total_pos for _loss, total_pos in zip(losses_bbox, num_total_pos)])
        losses_iou = sum([_loss.sum(dim=-1) / total_pos for _loss, total_pos in zip(losses_iou, num_total_pos)])

        # compute target loss --> sum
        loss_target = losses_cls + losses_bbox + losses_iou + enc_loss_cls + enc_losses_bbox + enc_losses_iou
    ...
    def loss_single(self,
        ...
        loss_cls = loss_cls.reshape(num_imgs, -1, loss_cls.shape[-1])
        loss_bbox = loss_bbox.reshape(num_imgs, -1, loss_bbox.shape[-1])
        try:
